...
export default function Searcher () {
    ...
    const [resList, setResList] = useState(['first']);
    ...
    //line 64 might break the code if resList.length === 0

    return (
        <>
            {isLoading && <Loading />}
            {!isLoading && resList[0] !== 'first' && (resList.length > 0 ? resList.map((store) => (
                <>
                    ...
                    {store[1].map(item => (
                        <div key={item.name} style={{animationDelay: `calc(.25s)`}} className='item'>
                            ...
                            {item.name} ...... ${item.price} -- {item.perUnit}
                        </div>
                    ))}
                </>
            )) : 
            ...
            <div className='search'>
                <div className='storeList'>
                    <div className='storeSelection' 
                        style={filters.kroger ? {backgroundColor: '#595', color: '#212121'} : {backgroundColor: '#637363', color: '#aaa'}} 
                        onClick={() => setFilters({...filters, kroger: !filters.kroger})}>
                            Kroger
                    </div>
                    <div className='storeSelection' 
                        style={filters.harristeeter ? {backgroundColor: '#595', color: '#212121'} : {backgroundColor: '#637363', color: '#aaa'}} 
                        onClick={() => setFilters({...filters, harristeeter: !filters.harristeeter})}>
                            Harris Teeter
                    </div>
                    <div className='storeSelection' 
                        style={filters.traderjoes ? {backgroundColor: '#595', color: '#212121'} : {backgroundColor: '#637363', color: '#aaa'}} 
                ...
                <div className='searchContainer'>
                    ...
                    <button className='searchButton' 
                        ...
                        style={{transition: 'background-color .4s ease-out, border-color .4s ease-out, color .4s ease-out', borderColor: filters.keywords && '#333', color: filters.keywords && '#212121', backgroundColor: filters.keywords ? '#595' : '#637363'}}
